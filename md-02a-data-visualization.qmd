...
mapply(setdiff, gapminder_2007, read_rds(here::here("data/gapminder-2007.rds")))
mapply(setdiff, gapminder_summary_2007, read_rds(here::here("data/gapminder-summary-2007.rds")))
mapply(setdiff, gapminder_summary_continent_year, read_rds(here::here("data/gapminder-summary-continent-year.rds")))
```
...
# https://stackoverflow.com/a/78066917
ggplot(gapminder_2007, aes(continent, lifeExp, fill = continent)) +
  geom_boxplot(outliers = FALSE) +
  theme_linedraw()

# data types
# https://stackoverflow.com/a/45755735
types <- lapply(gapminder_2007, class)
str(types)

# variables used: continent, lifeExp and continent
str(c(types[2], types[4], types[2]))
```
...
ggplot(gapminder_summary_continent_year, aes(year, lifeExp, color = continent)) +
  geom_line() +
  geom_point() +
  theme_linedraw()
```
...
ggplot(gapminder_summary_2007, aes(continent, count, fill = continent)) +
  geom_col() +
  theme_linedraw()
```
...
ggplot(gapminder_2007, aes(continent, fill=continent)) +
  geom_bar() +
  theme_linedraw()

# code is equivalent as we are now using gapminder_2007, geom_bar just counts how many data points per element of a class, in our case, continents. It is a histogram function with automatic binning dictated by the class structure.
```
...
ggplot(gapminder_2007, aes(lifeExp, fill = continent)) +
  geom_histogram(col = "grey30", breaks = seq(40, 85, 2.5)) +
  theme_linedraw()
# geom_bar works with discrete variables that don't need binning, geom_histogram works with continuos variables.
# breaks determines the binning, min and max values are 40 and 85, 2.5 is the size of a bin.
```
...
ggplot(gapminder_2007, aes(gdpPercap, lifeExp, size=pop, color=country)) +
